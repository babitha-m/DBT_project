...
# Create Kafka consumer for city_c topic
consumer_c = KafkaConsumer('Chennai', bootstrap_servers=bootstrap_servers,
                           value_deserializer=lambda x: json.loads(x.decode('utf-8')))

# Consume messages from topic and process with Spark SQL
for message in consumer_c:
    data = message.value
    ...
    print(f"City C - Average CO: {result['avg_co']}, Average Humidity: {result['avg_humidity']}, Average LPG: {result['avg_lpg']}, Average Smoke: {result['avg_smoke']}, Average Temp: {result['avg_temp']}")
    print(f"City C - Min CO: {result['min_co']}, Min Humidity: {result['min_humidity']}, Min LPG: {result['min_lpg']}, Min Smoke: {result['min_smoke']}, Min Temp: {result['min_temp']}")
    print(f"City C - Max CO: {result['max_co']}, Max Humidity: {result['max_humidity']}, Max LPG: {result['max_lpg']}, Max Smoke: {result['max_smoke']}, Max Temp: {result['max_temp']}")
